...
## 功能
- 讀取貼上輸入或專案根目錄的 `libs.txt`（選項 2/3 固定使用此檔，無需再提供自訂 TXT 路徑）
...
- 選項 3：直接使用與 `LIB_SEARCH.py` 同目錄的 `libs.txt`，不會詢問覆蓋或暫停；若檔案不存在會建立模板並提示重跑。此模式不連線 API，每筆輸入都輸出 `NO RESULT`，但輸出版型與其他模式一致。
